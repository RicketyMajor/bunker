...
def inventory_action(request):
    ...
    Todas las operaciones están envueltas en transaction.atomic() para
    prevenir ítems fantasma por fallos parciales.
    """
    ...
    try:
        with transaction.atomic():
            slot = InventorySlot.objects.select_for_update().get(id=slot_id)
            guild = GuildProfile.objects.select_for_update().get(id=1)
            ...
            if action == "to_guild":
                ...
                target_adv = Adventurer.objects.select_for_update().get(id=adv_id)
                ...
                adv = Adventurer.objects.select_for_update().get(id=slot.adventurer.id)
                item = slot.item
                ...
                if slot.adventurer:
                    recipient = Adventurer.objects.select_for_update().get(id=slot.adventurer.id)
                else:
                    recipient = guild
